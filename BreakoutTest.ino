...
void drawBall(int x, int y, int xold, int yold, int ballsize) {
 ...
 /* if (xold <= x && yold <= y) {
    ...
    Paint_DrawRectangle(xold,yold,xold + ballsize, y,BLACK,1,1);
    //tft.fillRect(xold , yold, x - xold, ballsize, BLACK);
    Paint_DrawRectangle(xold,yold,x, yold+ballsize,BLACK,1,1);

  } else if (xold >= x && yold >= y) {
   // tft.fillRect(x + ballsize , yold, xold - x, ballsize, BLACK);
    Paint_DrawRectangle(x+ballsize ,yold, ballsize, yold+ballsize,BLACK,1,1);
    //tft.fillRect(xold , y + ballsize, ballsize, yold - y, BLACK);
    Paint_DrawRectangle(xold,y+ballsize,xold+ballsize, yold+ballsize,BLACK,1,1);
  } else if (xold <= x && yold >= y) {
    //tft.fillRect(xold , yold, x - xold, ballsize, BLACK);
    Paint_DrawRectangle(xold,yold,x, yold+ballsize,BLACK,1,1);
    //tft.fillRect(xold , y + ballsize, ballsize, yold - y, BLACK);
    Paint_DrawRectangle(xold,y+ballsize,ballsize, yold+ballsize,BLACK,1,1);
  } else if (xold >= x && yold <= y) {
    //tft.fillRect(xold , yold, ballsize, y - yold, BLACK);
    Paint_DrawRectangle(xold,yold,xold + ballsize, y,BLACK,1,1);
    //tft.fillRect(x + ballsize, yold, xold - x, ballsize, BLACK);
    Paint_DrawRectangle(x+ballsize ,yold, xold+ballsize, yold+ballsize,BLACK,1,1);
  }*/
...
int readUiSelection(game_type * game, game_state_type * state, const int16_t lastSelected ) {
  ...
  if (tp.z > MINPRESSURE && tp.z < MAXPRESSURE) {
    xpos = map(tp.x, TS_RT, TS_LEFT, 0, 320);
    ypos = map(tp.y, TS_BOT, TS_TOP, 0, 170);
